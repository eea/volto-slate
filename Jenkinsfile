        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
              ...
              sh script: '''sed -i "s#/opt/frontend/my-volto-project/src/addons/${GIT_NAME}/##g" cypress-coverage/coverage/lcov.info''', returnStatus: true
              archiveArtifacts artifacts: 'cypress-coverage/coverage/lcov.info', fingerprint: true
              sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.javascript.lcov.reportPaths=./cypress-coverage/coverage/lcov.info -Dsonar.sources=./src -Dsonar.projectKey=$GIT_NAME-$BRANCH_NAME -Dsonar.projectVersion=$BRANCH_NAME-$BUILD_NUMBER"
