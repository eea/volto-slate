...
/**
 * Move up a list with with `Ctrl+Up`. (The Up key is supposed here to be pressed.)
 * @param {Editor} Editor
...
export function moveListItemUp({ editor, event }) {
  ...
  // Else prevent the default behavior of Slate, React and DOM and stop the propagation of this event.
  ...
  const [match] = Editor.nodes(editor, {
    ...
    // * 'all' - take all the nodes from the root Editor node to the selected leaf Text node
    // * 'highest' - take the root Editor node (if there is a selection, in case no `at` option is given)
    // * 'lowest' - take the farthest-from-root selected leaf Text node
    // (replace the "leaf Text node" expressions in the above list with whatever matching function you use)
    mode: 'lowest',
  });
  ...
  // TODO: handle the exception that this can throw, when `at` has the last number in it a `0`. This case is possible because the condition above that uses the Array.prototype.reduce method checks only the sum but the last 2 items can be [2, 0], their sum is 2 but the last item is 0, so the exception is thrown.
  ...
  // Move the Node in the selection that is LI and is farthest-from-root to the existing Path just before its current location.
