 ...
 */
export const parent = (editor, root, at, options = {}) => {
  const _path = path(editor, root, at, options);
...
export const previous = (editor, root, options = {}) => {
  ...
  if (match == null) {
    if (Path.isPath(at)) {
      const [_parent] = parent(editor, root, at);
...
export const apply = (editor, root, op) => {
  ...
  normalizeNodes(editor, root.children);
