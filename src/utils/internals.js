 ...
 * @returns {boolean} Whether it refers to a (leaf) Text node in the
 * closest-to-root level it can be.
 */
 ...
 * closest-to-root level a Point can be (a Point can only reffer to Text nodes).
 ...
 * level 0, blocks - level 1, blocks and leaves - level 2, it is level 2) then
 * the cursor specified by the given range can include inner node childs and the
 * return value can be true at the same time. Practically the cursor is always
 * at block level since nothing exists outside blocks in Slate and is always in
 * Text nodes since a Point can only reffer to an offset in a Text node.
